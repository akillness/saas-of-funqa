...
> Firebase App Hosting + Google Genkit 기반 RAG SaaS 모노레포.
...
```
Browser ──► Next.js (App Hosting) ──► Express API (Genkit)
                                            │
                              ┌─────────────┼─────────────┐
                              ▼             ▼             ▼
...
| AI | Google Gemini (`gemini-embedding-001`) |
| Database | Firebase Firestore |
| Auth | Firebase Auth (Google Login) |
| Infra | Firebase App Hosting |
...
| `EMBEDDING_MODEL_ID` | 임베딩 모델 ID | `gemini-embedding-001` |
...
API 서버와 Firebase Emulator를 동시에 실행합니다.
...
3. `firebase deploy --only apphosting:funqa-web` — Firebase 배포
...
| `npm run dev:apphosting` | Firebase 에뮬레이터 (App Hosting + Auth + Firestore) |
...
- 임베딩 모델은 `gemini-embedding-001` (Google 공식 호스팅 임베딩 엔드포인트)을 기본으로 사용합니다.
- `packages/ai`의 임베딩 어댑터는 플러그인 방식으로 교체 가능합니다.
- 로컬 검증 경로는 결정론적 해시 임베딩 백엔드를 사용하여 외부 모델 호출 없이 테스트할 수 있습니다.
