...
cells = [
md("# 6장 과제: Train/Test Split을 적용한 Ridge 회귀 성능 평가\n**과목**: 인공지능기반 데이터분석 | **제출자**: 정장영\n\n---\n**분석 방법**: Ridge Regression | **데이터**: Hitters (ISL)"),

md("## 1. 환경 설정 및 패키지 설치"),
code("""!pip install ISLP -q
import numpy as np
import pandas as pd
...
md("## 3. Train/Test Split (set.seed=42, 70/30)\n\n**재현성**: `np.random.seed(42)`를 맨 앞에 위치"),
code("""# 재현성 보장
np.random.seed(42)
...
md("## 5. 최종 모델 적합 (X_train 전체 사용)"),
code("""# 최적 lambda로 최종 모델 적합
...
out = BASE / "6장과제_정장영.ipynb"
out.write_text(json.dumps(nb, ensure_ascii=False, indent=2))
print(f"노트북 생성: {out}  ({len(cells)} cells)")
