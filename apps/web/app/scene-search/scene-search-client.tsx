...
import { AgentActivityOrb, FocusBeam } from "@/components/motion";
...
export function SceneSearchClient({
  ...
  const copy = useMemo(
    () =>
      isKo
        ? {
            title: "영상 장면을 질문하고, 타임코드 근거를 확인하세요.",
            lede: "관리자가 검증한 영상·FunQA 페어를 텍스트 또는 선택한 질의 프레임으로 검색하고, 인용 가능한 장면만으로 답합니다.",
            emptyBadge: "업로드 대기",
            localBadge: "로컬 프레임",
            liveBadge: "Genkit 실측",
            ...
            emptyPlayer: "검색 결과 프레임이 여기에 표시됩니다",
            emptyPlayerHint:
              "먼저 위에서 질문하세요. 필요할 때만 질의 영상을 선택해 프레임 검색을 함께 사용합니다.",
            timeline: "근거 타임라인",
            summary: "분석 요약",
        ...
        : {
            title: "Ask about a scene and inspect timestamped evidence.",
            lede: "Search the admin-reviewed video and FunQA pairs with text or optional query frames, then answer only from citable scenes.",
            ...
            emptyPlayer: "The top result frame appears here",
            emptyPlayerHint:
              "Ask a question above. Choose a query video only when you want to add frame-based search.",
            ...
            queryTitle: "Search video evidence",
  ...
  return (
    <div className="vqa-workspace">
      ...
      <div className="vqa-search-tools">
        <section className="vqa-query" aria-label={copy.queryTitle}>
          ...
          <div className="vqa-query-composer">
            <input
              ...
              value={query}
            />
            <FocusBeam active={searching} className="vqa-query-submit-beam">
              <button
          ...
          {!user && !authLoading ? (
            <p className="scene-note">
              {isKo ? "검색하려면 로그인하세요." : "Sign in to search."}{" "}
              <Link href={loginHref}>{isKo ? "로그인 →" : "Sign in →"}</Link>
            </p>
          ) : null}
          {searchError ? (
          ...
          <p aria-live="polite" className="sr-only" role="status">
            {searching ? copy.searching : resultsAnnouncement}
          </p>
        </section>
      ...
      {sourceFrames.length || searchResult ? (
        <>
              ...
              <div className="vqa-player-stage">
                {previewUrl ? (
                  <video
                    ...
                    src={previewUrl}
                  />
                ) : findingResult?.imageDataUrl ? (
                  <figure className="vqa-result-stage">
                    <img alt={findingResult.caption} src={findingResult.imageDataUrl} />
                    <figcaption>
                      <span>{formatVideoTimecode(findingResult.timecodeSec)}</span>
                      <strong>{findingResult.documentTitle}</strong>
                      <p>{findingResult.caption}</p>
                    </figcaption>
                  </figure>
                ) : (
                            ...
                            <li key={citation.sceneId}>
                              {canSeek ? (
                                ...
                                >
                                  ...
                                  {formatVideoTimecode(citation.timecodeSec)} · {copy.seek}
                                </button>
                              ) : (
                                <span>
                                  {citation.documentTitle} ·{" "}
                                  {formatVideoTimecode(citation.timecodeSec)}
                                </span>
                              )}
                            </li>
                          );
                    ...
                    ? searchResult.queryCaptions.map((caption, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span>
                              <strong>
                                {formatVideoTimecode(frame?.timecodeSec ?? 0)} ·{" "}
                                {copy.observedStatus}
                              </strong>
                              <p>{caption}</p>
                            </span>
                      ...
                      return (
                        <li key={result.sceneId}>
                          <button
                            ...
                            type="button"
                          >
                            <span className="vqa-evidence-image">
                              <img
                            ...
                            <span className="vqa-evidence-body">
                              <span className={`vqa-confidence vqa-confidence--${confidence}`}>
                                {confidence}
                              </span>
                              <strong>{result.documentTitle}</strong>
                              <span className="vqa-evidence-text">{result.caption}</span>
                              {result.analysisEvidence ? (
                                <span className="vqa-evidence-text">
                                  <b>
