...
Currently configured project: `saas-of-funqa` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/saas-of-funqa/SETUP.md` to fill in
   `data/saas-of-funqa/INFO.md`.
...
`--project-id saas-of-funqa` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/saas-of-funqa/`.
